		...
		else
		{
			if(initConfig.tidymode){
				...
				if(showclass=="onCorrect"){
					tip.parent(".dialog_box").hide();
					tip.parent().siblings(".right_img").show();	
				}else{
					tip.show().removeClass().html( showmsg );
					tip.parent(".dialog_box").show();
					tip.parent().siblings(".right_img").hide();
				}
...
$.fn.regexValidator = function(controlOptions)
{
	var settings = 
	{
		...
		comparetype : "||",
		onerror:"输入的格式不正确",
		valiregexValidatordatetype:"RegexValidator"
	};
	controlOptions = controlOptions || {};
	$.extend(true, settings, controlOptions);
	return this.each(function(){
